...
@okthon.on(events.NewMessage(outgoing=True, pattern=r"\.بوت المليار"))
async def _(event):
    if ispay[0] == "yes":
...
@okthon.on(events.NewMessage(outgoing=True, pattern=r"\.جمع"))
async def _(event):
    if ispay[0] == "yes":
        ...
        for i in range(100):
            ...
            try:
                try:
                    await okthon(JoinChannelRequest(url))
                except:
                    bott = url.split('/')[-1]
                    await okthon(ImportChatInviteRequest(bott))
